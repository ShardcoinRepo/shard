...
{
    QString ret = QString("shard:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SHARD unit input in lnReqAmount, we generate the URI with SHARD as unit (as defined in BIP21)
